...
class ucp_dkp extends \bbguild\admin\Admin
{
   ...
   /**
    ...
    function main($id, $mode)
    {
        ...
        switch ($mode)
        {
            case 'characters':
                ...
                $member = new \bbguild\controller\members\Members();
                if ($submit)
                {
            ...
            case 'characteradd':
                ...
                if ( $submit || $update || $delete )
                {
                    if($delete)
                    {
                        ...
                        if (confirm_box(true))
                        {
                            $deletemember = new \bbguild\controller\members\Members();
                    ...
                    if($submit)
                    {
                        ...
                        $newmember = new \bbguild\controller\members\Members();
                        if($newmember->has_reached_maxbbguildaccounts())
                        {
                            trigger_error(sprintf($user->lang['MAX_CHARS_EXCEEDED'],$config['bbguild_maxchars']) , E_USER_WARNING);
                        }
    ...
    /**
     * @param $member_id
     * @return \bbguild\controller\members\Members
     */
    private function UpdateMyCharacter($member_id)
    {
        $updatemember = new \bbguild\controller\members\Members();
        ...
        $oldmember = new \bbguild\controller\members\Members();
    ...
    private function fill_addmember($member_id, $guildlist)
    {
        ...
        $members = new \bbguild\controller\members\Members();

        // Attach the language file
        $user->add_lang('mods/common');
        $user->add_lang(array('mods/admin'));
        $show=true;

        if($member_id == 0)
        {
            // check if user can add character
            if(!$auth->acl_get('u_charadd') )
            {
            ...
            if($members->has_reached_maxbbguildaccounts())
            {
                $show=false;
                $template->assign_vars(array(
                    'MAX_CHARS_EXCEEDED' => sprintf($user->lang['MAX_CHARS_EXCEEDED'],$config['bbguild_maxchars']),
                ));
        ...
        foreach ($guildlist as $g)
        {
            ...
            $guilds = new \bbguild\controller\guilds\Guilds($members->member_guild_id);
        ...
        $Ranks = new \bbguild\controller\guilds\Ranks($members->member_guild_id);
    ...
    private function listmychars()
    {
        ...
        foreach ($mycharacters[0] as $char)
        {
            ...
            $sql_array2 = array(
			    ...
			    SUM(m.member_spent - m.member_item_decay + ( ' . max(0, $config['bbguild_basegp']) . ')) AS gp,
     			SUM(m.member_earned + m.member_adjustment - m.member_spent + m.member_item_decay - ( ' . max(0, $config['bbguild_basegp']) . ') ) AS member_current,
				CASE WHEN SUM(m.member_spent - m.member_item_decay) <= 0
					THEN SUM(m.member_earned + m.member_adjustment)
					ELSE ROUND( SUM(m.member_earned + m.member_adjustment) /  SUM(' . max(0, $config['bbguild_basegp']) .' + m.member_spent - m.member_item_decay) ,2)
				END AS pr',
                'FROM'      => array(
                    MEMBER_DKP_TABLE 	=> 'm',
                    DKPSYS_TABLE 		=> 'd',
            ...
            {
                $template->assign_block_vars('members_row.row', array(
        ...
        $template->assign_vars(array(
            'S_SHOWEPGP' 	=> ($config['bbguild_epgp'] == '1') ? true : false,
        ));

...
class Events extends Admin
{
    ...
    public function get($event_id = 0)
    {
        ...
        $sql = 'SELECT b.dkpsys_name, b.dkpsys_id, a.event_name, a.event_value,
				...
				FROM ' . BBEVENTS_TABLE . ' a, ' . DKPSYS_TABLE . " b
    ...
    public function add()
    {
        global $user, $db;
        // check existing
        $result = $db->sql_query("SELECT count(*) as evcount from " . BBEVENTS_TABLE .
        ...
        $db->sql_query('INSERT INTO ' . BBEVENTS_TABLE . $query);
     ...
     */
    public function update(Events $oldevent)
    {
        global $user, $db;
        ...
        $sql = 'UPDATE ' . BBEVENTS_TABLE . ' SET ' . $query . ' WHERE event_id=' . (int) $this->event_id;
        $db->sql_query($sql);
    ...
    public function delete()
    {
        ...
        $sql = 'DELETE FROM ' . BBEVENTS_TABLE . ' WHERE event_id = ' . $this->event_id ;
        $db->sql_query($sql);
    ...
    {
        global $db;
        $sql = 'SELECT count(*) as eventcount FROM ' . DKPSYS_TABLE . ' a , ' . BBEVENTS_TABLE . " b
    ...
    public function getmax($dkpid = 0)
    {
        ...
        $sql = 'SELECT max(event_value) AS max_value FROM ' . BBEVENTS_TABLE . ' where event_status = 1 AND event_dkpid = ' . $this->dkpsys_id;
        $result = $db->sql_query ($sql);
    ...
    public function viewlistevents($guild_id)
    {
        ...
        while ( $pool = $db->sql_fetchrow($dkppool_result) )
        {
            ...
            $start = $request->variable('pool'. $pool['dkpsys_id'], 0);

...
class dkp_game_module extends Admin
{
	...
	function main($id, $mode)
	{
		...
		switch ($mode)
		{
			case 'listgames' :
				...
				$this->link = '<br /><a href="' . append_sid ( "{$phpbb_admin_path}index.$phpEx", 'i=\sajaki\bbdkp\acp\dkp_game_module&amp;mode=listgames' ) . '"><h3>' . $user->lang['RETURN_GAMELIST'] . '</h3></a>';
				...
				foreach($this->gamelist as $game_id => $game)
			    {
			    	$template->assign_block_vars('gamerow', array(
			    			...
			    			'U_VIEW_GAME' => append_sid("{$phpbb_admin_path}index.$phpEx", 'i=\sajaki\bbdkp\acp\dkp_game_module&amp;mode=editgames&amp;' . URI_GAME . '=' . $game['game_id'] ),
			    			'STATUS' => $game['status'],
			    	));
			    }

			    $template->assign_vars ( array (
			    		'U_LIST_GAME' => append_sid("{$phpbb_admin_path}index.$phpEx", 'i=\sajaki\bbdkp\acp\dkp_game_module&amp;mode=listgames') ,
			...
			case 'editgames' :
				...
				$this->link = '<br /><a href="' . append_sid ( "{$phpbb_admin_path}index.$phpEx", 'i=\sajaki\bbdkp\acp\dkp_game_module&amp;mode=editgames&amp;' . URI_GAME ."={$editgame->game_id}" ) . '"><h3>' .
                ...
                $template->assign_vars ( array (
                    'U_BACK' => append_sid("{$phpbb_admin_path}index.$phpEx", 'i=\sajaki\bbdkp\acp\dkp_game_module&amp;mode=listgames') ,
                ));
    ...
    private function ResetGame(Game $editgame, $request)
    {
        ...
        if (confirm_box(true))
        {
            ...
            $editgame->Delete();
            $editgame->install();
            meta_refresh(1, append_sid("{$phpbb_admin_path}index.$phpEx", 'i=\sajaki\bbdkp\acp\dkp_game_module&amp;mode=listgames'));
    ...
    private function AddRole(Roles $role, Game $editgame, $request)
    {
        ...
        meta_refresh(1, append_sid("{$phpbb_admin_path}index.$phpEx", 'i=\sajaki\bbdkp\acp\dkp_game_module&amp;mode=editgames&amp;' . URI_GAME . "={$role->game_id}"));
    ...
    private function EditRole(Roles $role, Game $editgame, $request)
    {
        ...
        meta_refresh(1, append_sid("{$phpbb_admin_path}index.$phpEx", 'i=\sajaki\bbdkp\acp\dkp_game_module&amp;mode=editgames&amp;' . URI_GAME . "={$newrole->game_id}"));
    ...
    private function DeleteRole(Game $editgame, $request)
    {
        ...
        if (confirm_box(true))
        {
            ...
            meta_refresh(1, append_sid("{$phpbb_admin_path}index.$phpEx", 'i=\sajaki\bbdkp\acp\dkp_game_module&amp;mode=editgames&amp;' . URI_GAME . "={$deleterole->game_id}"));
    ...
    private function AddFaction(Faction $faction, Game $editgame, $request)
    {
        ...
        $this->log_insert(array(
            ...
            'log_result' => 'L_SUCCESS',
            'log_action' => $log_action));
        meta_refresh(1, append_sid("{$phpbb_admin_path}index.$phpEx", 'i=\sajaki\bbdkp\acp\dkp_game_module&amp;mode=editgames&amp;' . URI_GAME . "={$faction->game_id}"));
    ...
    private function AddClass($request)
    {
        ...
        $newclass->tank       = '';
        $newclass->Make();
        meta_refresh(1, append_sid("{$phpbb_admin_path}index.$phpEx", 'i=\sajaki\bbdkp\acp\dkp_game_module&amp;mode=editgames&amp;' . URI_GAME . "={$newclass->game_id}"));
    ...
    private function DeleteClass(Game $editgame, $request)
    {
        ...
        if (confirm_box(true))
        {
            ...
            $deleteclass->get();
            $deleteclass->Delete();
            meta_refresh(1, append_sid("{$phpbb_admin_path}index.$phpEx", 'i=\sajaki\bbdkp\acp\dkp_game_module&amp;mode=editgames&amp;' . URI_GAME . "={$deleteclass->game_id}"));
    ...
    private function RaceUpdate($request)
    {
        ...
        $this->log_insert(array(
            ...
            'log_result' => 'L_SUCCESS',
            'log_action' => $log_action));
        meta_refresh(1, append_sid("{$phpbb_admin_path}index.$phpEx", 'i=\sajaki\bbdkp\acp\dkp_game_module&amp;mode=editgames&amp;' . URI_GAME . "={$race->game_id}"));
    ...
    private function DeleteRace(Game $editgame, $request)
    {
        ...
        if (confirm_box(true))
        {
            ...
            meta_refresh(1, append_sid("{$phpbb_admin_path}index.$phpEx", 'i=\sajaki\bbdkp\acp\dkp_game_module&amp;mode=editgames&amp;' . URI_GAME . "={$deleterace->game_id}"));
    ...
    private function BuildTemplateAddRace(Game  $editgame, $request)
    {
        ...
        $template->assign_vars(array(
            ...
            'S_ADD'                 => true,
            'U_ACTION'              => append_sid("{$phpbb_admin_path}index.$phpEx", 'i=\sajaki\bbdkp\acp\dkp_game_module&amp;mode=addrace'),
    ...
    private function BuildTemplateEditClass(Game $editgame, $request)
    {
        ...
        $template->assign_vars(array(
            ...
            'S_ADD'                => false,
            'U_ACTION'             => append_sid("{$phpbb_admin_path}index.$phpEx", 'i=\sajaki\bbdkp\acp\dkp_game_module&amp;mode=addclass'),
            'MSG_NAME_EMPTY'       => $user->lang ['FV_REQUIRED_NAME'],
    ...
    private function BuildTemplateFaction(Game $editgame, $request)
    {
        ...
        $template->assign_vars(array(
            ...
            'U_ACTION'              => append_sid("{$phpbb_admin_path}index.$phpEx", 'i=\sajaki\bbdkp\acp\dkp_game_module&amp;mode=addfaction'),
            'MSG_NAME_EMPTY'        => $user->lang ['FV_REQUIRED_NAME']));
        unset($races);
    ...
    private function BuildTemplateRole(Game $editgame, $request)
    {
        ...
        $template->assign_vars(array(
            ...
            'GAME_NAME'          => $editgame->getName(),
            'IS_ADD'             => $add,
            'U_ACTION'           => append_sid("{$phpbb_admin_path}index.$phpEx", 'i=\sajaki\bbdkp\acp\dkp_game_module&amp;mode=addrole&amp;game_id=' . $editgame->game_id),
            'MSG_NAME_EMPTY'     => $user->lang ['FV_REQUIRED_NAME']));
        unset($role);
	...
	private function showgame( Game $editgame)
	{
		...
		foreach($fa as $faction_id => $faction)
		{
			...
			'U_DELETE' => append_sid ( "{$phpbb_admin_path}index.$phpEx",
                    'i=\sajaki\bbdkp\acp\dkp_game_module&amp;mode=editgames&amp;action=deletefaction&amp;id=' . $faction['f_index'] . '&amp;' . URI_GAME . '=' . $editgame->game_id),
			'U_EDIT' => append_sid ( "{$phpbb_admin_path}index.$phpEx",
            'i=\sajaki\bbdkp\acp\dkp_game_module&amp;mode=editgames&amp;action=editfaction&amp;id=' . $faction['f_index'] . '&amp;' . URI_GAME . '=' . $editgame->game_id),
        ));
		}
		...
		foreach ( $ra as $race_id => $race )
		{
			...
			$template->assign_block_vars ( 'race_row', array (
				...
				'U_VIEW_RACE' => append_sid ( "{$phpbb_admin_path}index.$phpEx", 'i=\sajaki\bbdkp\acp\dkp_game_module&amp;mode=addrace&amp;r=' . $race['race_id'] . '&amp;' . URI_GAME ."={$listraces->game_id}" ),
		...
		foreach ( $cl as $c_index => $class )
		{
			...
			$template->assign_block_vars ( 'class_row', array (
				...
				'U_VIEW_CLASS' => append_sid ( "{$phpbb_admin_path}index.$phpEx", 'i=\sajaki\bbdkp\acp\dkp_game_module&amp;mode=addclass&amp;r=' . $class ['class_id'] . '&amp;game_id=' . $listclasses->game_id ),
				'U_DELETE' => append_sid ( "{$phpbb_admin_path}index.$phpEx", 'i=\sajaki\bbdkp\acp\dkp_game_module&amp;mode=editgames&amp;classdelete=1&amp;id=' . $class['class_id'] . '&amp;game_id=' . $listclasses->game_id ),
				'U_EDIT' => append_sid ( "{$phpbb_admin_path}index.$phpEx", 'i=\sajaki\bbdkp\acp\dkp_game_module&amp;mode=editgames&amp;classedit=1&amp;id=' . $class['class_id'] . '&amp;game_id=' . $listclasses->game_id ) ) );
		}
		...
		$template->assign_vars ( array (
				...
				'U_ADD_GAMES' => append_sid ( "{$phpbb_admin_path}index.$phpEx", 'i=\sajaki\bbdkp\acp\dkp_game_module&amp;mode=editgames&amp;' ),

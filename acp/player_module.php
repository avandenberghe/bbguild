...
class player_module extends Admin
{
        ...
        switch ($mode)
        {
            ...
            case 'listplayers':
                ...
                if ($showadd)
                {
                    ...
                    redirect(append_sid("{$phpbb_admin_path}index.$phpEx", 'i=\bbdkp\bbguild\acp\mm_module&amp;mode=addplayer&amp;guild_id=' . $a ));
                    break;
                }
            ...
            case 'addplayer' :
                ...
                $this->link = '<br /><a href="' . append_sid("{$phpbb_admin_path}index.$phpEx", 'i=\bbdkp\bbguild\acp\mm_module&amp;mode=listplayers') . '"><h3>' . $this->user->lang['RETURN_PLAYERLIST'] . '</h3></a>';
    ...
    private function Addplayer()
    {
        ...
        if ($newplayer->player_id > 0)
        {
            ...
            meta_refresh(2, append_sid("{$phpbb_admin_path}index.$phpEx", 'i=\bbdkp\bbguild\acp\mm_module&amp;mode=listplayers&amp;' . URI_GUILD . "=" . $newplayer->player_guild_id));
            $success_message = sprintf($this->user->lang['ADMIN_ADD_PLAYER_SUCCESS'], ucwords($newplayer->player_name), date("F j, Y, g:i a"));

            $this->link = '<br /><a href="' . append_sid("{$phpbb_admin_path}index.$phpEx", 'i=\bbdkp\bbguild\acp\mm_module&amp;mode=listplayers&amp;' . URI_GUILD . "=" . $newplayer->player_guild_id) . '"><h3>' . $this->user->lang['RETURN_PLAYERLIST'] . '</h3></a>';
            trigger_error($success_message . $this->link, E_USER_NOTICE);
    ...
    private function UpdatePlayer()
    {
        ...
        meta_refresh(1, append_sid("{$phpbb_admin_path}index.$phpEx", 'i=\bbdkp\bbguild\acp\mm_module&amp;mode=listplayers&amp;' . URI_GUILD . "=" . $updateplayer->player_guild_id));
        $this->link = '<br /><a href="' . append_sid("{$phpbb_admin_path}index.$phpEx", 'i=\bbdkp\bbguild\acp\mm_module&amp;mode=listplayers&amp;' . URI_GUILD . "=" . $updateplayer->player_guild_id) . '"><h3>' . $this->user->lang['RETURN_PLAYERLIST'] . '</h3></a>';
    ...
    private function DeletePlayer()
    {
        ...
        meta_refresh(1, append_sid("{$phpbb_admin_path}index.$phpEx", 'i=\bbdkp\bbguild\acp\mm_module&amp;mode=listplayers&amp;' . URI_GUILD . "=" . $deleteplayer->player_guild_id));
        $this->link = '<br /><a href="' . append_sid("{$phpbb_admin_path}index.$phpEx", 'i=\bbdkp\bbguild\acp\mm_module&amp;mode=listplayers&amp;' .
    ...
    private function BuildTemplateListPlayers($mode, Guilds $Guild)
    {
        ...
        while ($row = $this->db->sql_fetchrow($players_result))
        {
            ...
            $this->template->assign_block_vars('players_row', array(
                ...
                'U_VIEW_PLAYER'        => append_sid("{$phpbb_admin_path}index.$phpEx", 'i=\bbdkp\bbguild\acp\mm_module&amp;mode=addplayer&amp;' . URI_NAMEID . '=' . $row['player_id']),
                'U_DELETE_PLAYER'      => append_sid("{$phpbb_admin_path}index.$phpEx", 'i=\bbdkp\bbguild\acp\mm_module&amp;mode=addplayer&amp;delete=1&amp;' . URI_NAMEID . '=' . $row['player_id'])));
        ...
        $pagination_url = append_sid("{$phpbb_admin_path}index.$phpEx",
            'i=\bbdkp\bbguild\acp\mm_module&amp;mode=listplayers&amp;o=' . $current_order['uri']['current'] .
        ...
        $this->template->assign_vars(array(
            ...
            'F_PLAYERS'             => append_sid("{$phpbb_admin_path}index.$phpEx", 'i=\bbdkp\bbguild\acp\mm_module') . '&amp;mode=addplayer',
            'F_PLAYERS_LIST'        => append_sid("{$phpbb_admin_path}index.$phpEx", 'i=\bbdkp\bbguild\acp\mm_module') . '&amp;mode=listplayers',
            'L_TITLE'               => $this->user->lang['ACP_MM_LISTPLAYERS'],
            'L_EXPLAIN'             => $this->user->lang['ACP_MM_LISTPLAYERS_EXPLAIN'],
            'O_NAME'                => append_sid("{$phpbb_admin_path}index.$phpEx", 'i=\bbdkp\bbguild\acp\mm_module&amp;mode=listplayers&amp;o=' . $current_order['uri'][0] . "&amp;" . URI_GUILD . "=" . $Guild->guildid . "&amp;minlevel=" . $minlevel .
                "&amp;maxlevel=" . $maxlevel .
                "&amp;active=" . $selectactive .
                "&amp;nonactive=" . $selectnonactive),
            'O_USERNAME'            => append_sid("{$phpbb_admin_path}index.$phpEx", 'i=\bbdkp\bbguild\acp\mm_module&amp;mode=listplayers&amp;o=' . $current_order['uri'][1] . "&amp;" . URI_GUILD . "=" . $Guild->guildid . "&amp;minlevel=" . $minlevel .
                "&amp;maxlevel=" . $maxlevel .
                "&amp;active=" . $selectactive .
                "&amp;nonactive=" . $selectnonactive),
            'O_LEVEL'               => append_sid("{$phpbb_admin_path}index.$phpEx", 'i=\bbdkp\bbguild\acp\mm_module&amp;mode=listplayers&amp;o=' . $current_order['uri'][2] . "&amp;" . URI_GUILD . "=" . $Guild->guildid . "&amp;minlevel=" . $minlevel .
                "&amp;maxlevel=" . $maxlevel .
                "&amp;active=" . $selectactive .
                "&amp;nonactive=" . $selectnonactive),
            'O_CLASS'               => append_sid("{$phpbb_admin_path}index.$phpEx", 'i=\bbdkp\bbguild\acp\mm_module&amp;mode=listplayers&amp;o=' . $current_order['uri'][3] . "&amp;" . URI_GUILD . "=" . $Guild->guildid . "&amp;minlevel=" . $minlevel .
                "&amp;maxlevel=" . $maxlevel .
                "&amp;active=" . $selectactive .
                "&amp;nonactive=" . $selectnonactive),
            'O_RANK'                => append_sid("{$phpbb_admin_path}index.$phpEx", 'i=\bbdkp\bbguild\acp\mm_module&amp;mode=listplayers&amp;o=' . $current_order['uri'][4] . "&amp;" . URI_GUILD . "=" . $Guild->guildid . "&amp;minlevel=" . $minlevel .
                "&amp;maxlevel=" . $maxlevel .
                "&amp;active=" . $selectactive .
                "&amp;nonactive=" . $selectnonactive),
            'O_LAST_UPDATE'         => append_sid("{$phpbb_admin_path}index.$phpEx", 'i=\bbdkp\bbguild\acp\mm_module&amp;mode=listplayers&amp;o=' . $current_order['uri'][5] . "&amp;" . URI_GUILD . "=" . $Guild->guildid . "&amp;minlevel=" . $minlevel .
                "&amp;maxlevel=" . $maxlevel .
                "&amp;active=" . $selectactive .
                "&amp;nonactive=" . $selectnonactive),
            'O_ID'                  => append_sid("{$phpbb_admin_path}index.$phpEx", 'i=\bbdkp\bbguild\acp\mm_module&amp;mode=listplayers&amp;o=' . $current_order['uri'][7] . "&amp;" . URI_GUILD . "=" . $Guild->guildid . "&amp;minlevel=" . $minlevel .
                "&amp;maxlevel=" . $maxlevel .
                "&amp;active=" . $selectactive .
                "&amp;nonactive=" . $selectnonactive),
            'U_LIST_PLAYERS'        => append_sid("{$phpbb_admin_path}index.$phpEx", 'i=\bbdkp\bbguild\acp\mm_module&amp;mode=listplayers&amp;'),
    ...
    private function BuildTemplateAddEditplayers($mode)
    {
        ...
        $this->template->assign_vars(array(
            ...
            'F_ADD_PLAYER'             => append_sid("{$phpbb_admin_path}index.$phpEx", 'i=\bbdkp\bbguild\acp\mm_module&amp;mode=addplayer&amp;'),

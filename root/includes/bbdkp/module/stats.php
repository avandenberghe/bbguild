...
memberstats($dkp_id, $time);
class_statistics($dkp_id);
attendance_statistics($dkp_id, $time);
...
{
	global $user, $db, $template, $phpEx, $phpbb_root_path, $query_by_pool, $u_stats;
 ...
 *
 */
function memberstats($dkp_id, $time)
{
	global $db, $template, $config, $phpEx, $phpbb_root_path, $user;
	
	global $query_by_pool, $u_stats;
	...
	$dkppagination = generate_pagination2($u_stats . '&amp;o1=' . $current_order ['uri'] ['current'] , 
	...
	$template->assign_vars(array(
	    ...
	    'O_NAME'       => append_sid("{$phpbb_root_path}dkp.$phpEx", 'page=stats&amp;1o=' . $current_order['uri'][2] . '&amp;' . URI_DKPSYS . '=' . ($query_by_pool ? $dkp_id : 'All')), 
 ...
 *
 */
function class_statistics($dkp_id)
{
	global $db, $config, $template, $phpEx, $phpbb_root_path;
	global $total_drops, $query_by_pool;
 ...
 */
function attendance_statistics($dkp_id, $time)
{
	...
	$attpagination = generate_pagination2($u_stats . '&amp;o=' . $att_current_order ['uri'] ['current'] , 
